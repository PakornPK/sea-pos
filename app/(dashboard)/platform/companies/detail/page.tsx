...
import type { InvoiceListRow, SubscriptionListRow } from '@/lib/repositories/contracts/billing'
...
type Plan = Awaited<ReturnType<typeof planRepo.listActive>>[number]
type Subscription = Awaited<ReturnType<typeof billingRepo.getSubscriptionByCompany>>
type Payment = Awaited<ReturnType<typeof billingRepo.listPaymentsBySubscription>>[number]
...
export default function CompanyDetailPage() {
  ...
  useEffect(() => {
    ...
    ]).then(async ([comp, pl, sub, invs]) => {
      setCompany(comp ?? null)
      setPlans(pl)
      setSubscription(sub)
      setInvoices(invs)
      if (sub) {
        const pays = await billingRepo.listPaymentsBySubscription(sub.id)
        setPayments(pays)
      }
    })
  }, [id])

  if (company === undefined) return null  // loading
  if (!company) notFound()

...
import { getPODetail, type PODetailData } from '@/lib/actions/purchasing'
...
export default function PODetailPage() {
  ...
  const [data, setData] = useState<PODetailData | null | undefined>(undefined)

  useEffect(() => {
    if (!id || !user) return
    getPODetail(id, user.companyId).then((d) => setData(d))
  }, [id, user])

  if (!user) return null  // AuthGuard handles redirect
  if (data === undefined) return null  // loading
  if (!data || !data.po) notFound()

  const { po, items, suppliers, products, categories, vatConfig, branch, signerOptions, lockedSignerName } = data

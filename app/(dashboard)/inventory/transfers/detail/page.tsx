...
import { getTransferDetail, type TransferDetail } from '@/lib/actions/stockTransfers'
...
export default function TransferDetailPage() {
  ...
  const [transfer, setTransfer] = useState<TransferDetail | null | undefined>(undefined)

  useEffect(() => {
    if (!id) return
    getTransferDetail(id).then(setTransfer)
  }, [id])

  if (!user) return null  // AuthGuard handles redirect
  if (transfer === undefined) return null  // loading
  if (transfer === null) notFound()

  const isManageRole = user.role === 'admin' || user.role === 'manager'
  const canReceive = isManageRole
    && transfer.status === 'in_transit'
    && (user.isPlatformAdmin || user.role === 'admin' || user.branchIds.includes(transfer.to_branch.id))

  const canCancel = isManageRole
    && (transfer.status === 'in_transit' || transfer.status === 'draft')
    && (user.isPlatformAdmin || user.role === 'admin' || user.branchIds.includes(transfer.from_branch.id))

...
import { getReceiptData, type ReceiptData } from '@/lib/actions/pos'
...
export default function ReceiptPage() {
  ...
  const { sale, items, company, branch } = data!
  if (!sale) notFound()
  ...
  const canVoid = !isVoided && (user.role === 'admin' || user.role === 'manager')
  ...
  const customerName = sale.member_name ?? null
    ...
    <div className="flex flex-col gap-4 max-w-sm mx-auto">
      ...
      >
        ...
        <div className="space-y-1 text-sm">
          ...
          {customerName && <Row label="สมาชิก" value={customerName} />}
        </div>
        ...
        <div className="space-y-2.5">
          {items.map((item) => {
            ...
            const basePrice = item.unit_price - opts.reduce((sum: number, o: { price_delta: number }) => sum + o.price_delta, 0)
            return (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {opts.length > 0 ? (
                    <ul className="mt-0.5 space-y-0 text-xs text-muted-foreground">
                      ...
                      {opts.map((o: { option_name: string; price_delta: number }, i: number) => (
